export default function Chat({ mine, system, username, text, time }) {
  if (system) {
    return (
    ...
    <div className={`flex ${mine ? 'justify-end' : 'justify-start'} my-1`}>
      <div
        className={`max-w-[75%] rounded-2xl px-3 py-2 text-sm shadow 
          ${mine ? 'bg-[#5d5cf5] text-white' : 'bg-[#2a2a2a] text-[#eaeaea]'}`}
      >
        {!mine && (
          <p className="text-[11px] font-semibold text-[#b3b3b3] mb-1">
            {username}
          </p>
        )}
        ...
        <div
          className={`mt-1 text-[10px] ${
            mine ? 'text-white/80 text-right' : 'text-[#b3b3b3]'
          }`}
        >
          {time}
        </div>
