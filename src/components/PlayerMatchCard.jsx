...
function PlayerMatchCard({ match, puuid }) {
  ...
  return (
    <div className={`${base} ${stateClass}`}>
      <div className="flex flex-col w-20 p-1 gap-1 text-xs justify-center">
        <div>{match.rank_type}</div>
        <div>{match.game_end_timestamp}</div>
        <div>{match.win ? '승리' : '패배'}</div>
        <div>{match.play_duration}</div>
      </div>

      <div className="flex flex-col w-20 items-center">
        <img
          src={getChampionImg(match?.champion_name)}
      ...
      <div className="flex flex-col flex-1 gap-1 text-xs">
        ...
        <div className="flex gap-1">
          {match.spells.map((id, idx) => (
            <img key={idx} src={getSpellImg(id)} className="w-6 h-6" />
          ))}
          <img
            src={getKeystoneImg(match.runes.primary_style)}
            className="w-6 h-6"
          />
          <img
            src={getKeystoneImg(match.runes.sub_style)}
            className="w-6 h-6"
          />
        </div>
        <div className="flex gap-1">
          {match.items.map((id, idx) => (
            <img key={idx} src={getItemImg(id)} className="w-6 h-6" />
          ))}
        </div>
      </div>
    </div>
